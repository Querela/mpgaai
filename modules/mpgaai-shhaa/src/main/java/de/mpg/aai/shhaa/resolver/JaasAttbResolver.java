 ...
 * takes the jaas config file SECTION name from the configuration and invokes
 * jaas:
 * enables client developers to add thus their own implementations to resolve
 * attributes
 * for the given Subject (and add them as {@link AuthAttribute} to the Subject's
 * public credentials
 * 
 * @see <a href=
...
public class JaasAttbResolver implements AttributeResolverModule, Configurable {
	 ...
	 * with a single {@link NameCallbackHandler} providing the given subject's
	 * username
	 * (to be used by the jaas LoginModules to retrieve the username in order to
	 * lookup attributes for/by it)
	 * 
