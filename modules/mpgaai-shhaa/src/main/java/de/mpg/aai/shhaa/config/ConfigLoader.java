...
public class ConfigLoader {
	...
	/**
	 * reads in and load the configuration from the given config-context (providing
	 * the config-file location)
	 * 
	 * @param configCtx config-context to load on (providing the config-file
	 *                  location)
	...
	private static void loadHandler(final Configuration result, final Element rootElem) {
		...
		// auto-login: default to true if not configured explicitly = standard (shib)
		// use-case
	...
	/**
	 * loads the rules extracted from the given parent element and adds them to the
	 * given (parent) target rule
	 * 
			...
			for (int ii = 0; ii < childs.getLength(); ii++) {
				...
				if (name.equalsIgnoreCase("rule")) { // logic-RULE
					...
					throw new ConfigurationException(
							"found invalid rule tag, expected 'rule', 'require' or 'miss', got  " + name);
				}
			}
	 ...
	 * 
	 * @param elem source to extract the data from
	 * @return found values (attribute value split on " ") of given elements
	 *         attribute 'methods'
	 */
	...
	/**
	 * provides the NodeList (elements) of all childs with the given tag-name from
	 * the given parent element
	 * 
	 * @param parent    the element to lookup the data in
	 * @param tag       name of the tag(s) to lookup
	 * @param mandatory flag to indicate whether an result is mandatory (Exception
	 *                  if not found) or not
	...
	/**
	 * provides the (fist) element (of all childs) with the given tag-name from the
	...
	/**
	 * provides the value of the (fist) element (of all childs) with the given
	 * tag-name from the given parent element
	 * 
	 * @param parent    the element to lookup the data in
	 * @param tag       name of the tag(s) to lookup
	 * @param mandatory flag to indicate whether an result is mandatory (Exception
	 *                  if not found) or not
	 * @return VALUE of the element representing the (first) found data, null if no
	 *         hit
	 * @throws ConfigurationException if mandatory and no result could be found
	 */
