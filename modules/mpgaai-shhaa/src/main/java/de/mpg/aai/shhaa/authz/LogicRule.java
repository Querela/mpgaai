 ...
 * <p>
 * syntax: {@code <rule logic="AND|OR">}
 * </p>
 * the attribute 'logic' specifies the behavior of how this rule's child rules
 * ({@link #childRules}
 * are interpreted during {@link #evaluate(AuthAttributes)}:
 * <ul>
 * <li>AND - all (child)rules need to match (themselves evaluate to true) to
 * evaluate (this) to true</li>
...
public class LogicRule implements Rule {
	 ...
	 * constructor to init the logic mode of this Logic-Rule: AND | OR
	 * 
	 * @param logicMode the destined mode as string, is parsed to boolean:
	 *                  {@link #parseLogicMode(String)}
	 ...
	 * @return true if mode is 'OR' (case insensitive), false if 'AND', null for any
	 *         other value (= could not parse)
	 * @throws
	 */
