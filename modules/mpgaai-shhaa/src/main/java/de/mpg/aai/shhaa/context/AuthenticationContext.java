...
public class AuthenticationContext extends AbstractReadOnly {
	/**
	 * the AAI session-ID: an aai session might differ from a (pure single webapp)
	 * session-id (like JSESSIONID)
	 */
	private String sessionID;
	/**
	 * name/id/label of the identity-provider the principal has
	...
	public AuthPrincipal getAuthPrincipal() {
		...
		// we expect that this iterator always provides the very same - first - element
		// from the set
		// <=> although Set is not specified as sorted, the actual impl in Subject IS
		// using a LinkedList
	...
	/**
	 * @return the AAI Attributes (UNBACKED reference!) from the Subject's (public)
	 *         credentials
	 */
	...
	/**
	 * @return whether this current context represents a fallback session/user
	 *         (&lt;=&gt; no valid shib-user/session)
	 */
	...
	public String toString() {
		...
		// result.append(" by ").append(this.subject != null ? this.subject.toString() :
		// "no-subject");
