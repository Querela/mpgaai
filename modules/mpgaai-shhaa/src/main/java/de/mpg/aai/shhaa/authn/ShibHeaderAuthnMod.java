...
public class ShibHeaderAuthnMod extends BaseAuthnMod implements AuthenticationModule, Configurable {
	...
	/**
	 * provides authentication context with data looked up from given requests
	 * shibboleth-SP http header attributes
	 * {@inheritDoc}
	 */
	@Override
	public void initAuthenticationContext(final AuthenticationContext authCtx, HttpServletRequest request)
		...
		// we use the shib-session-ID to determine whether there is a (valid) shib
		// session
		String shibSession = this.getHeader(this.getConfig().getShibSessionID(), request);
		// => no shib-session => not logged-in => provide fallback auth-context (incl
		// fallback user)
	...
	/**
	 * provides principal/subject with data looked up from given requests
	 * shibboleth-SP http header attributes
	 * {@inheritDoc}
	 */
	@Override
	...
	/**
	 * parses the given time string (from shibboleth-SP) into a proper
	 * java.util.Date instance
	 * 
	 * @param time string representation of a timestamp, formatted in shibboleth-SP
	 *             style
	 * @return
	 */
