 ...
 * to lookup shibboleth attributes from the request http header (shibboleth
 * service provider header attributes)
 * 
 * @author megger
 *
 */
public class ShibHeaderAttbResolver implements AttributeResolverModule, Configurable {
	...
	/**
	 * resolves (loads/retrieves/fetches) shibboleth attributes
	 * ({@link AuthAttribute}) for the given user
	...
	public void resolve(Subject user, HttpServletRequest request) {
		...
		for (String targetID : targets) {
			...
			for (Object val; values.hasMoreElements();) {
				...
				String[] vals = null;
				// expecting just strings => treating everything like one (an exception makes
				// not much sense: no cure)
