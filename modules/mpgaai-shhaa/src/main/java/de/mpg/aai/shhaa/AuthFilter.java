 ...
 * calls the main service, aai authn/z handler
 * {@link HttpAuthService#handleAuth(HttpServletRequest, HttpServletResponse)}
 * and continues/forwards/redirects the original request to the proper location
...
public class AuthFilter implements Filter {
	 ...
	 * and continues/forwards/redirects the original request to the proper location
	 * depending on the returned result:
	 * if returned {@link AuthenticationContext}'s target starts with http the
	 * resulted request is redirected to the given url,
		...
		if (target.startsWith("http")) { // redirect
			...
			if (dispatch == null)
				throw new NullPointerException(
						"could not acquire dispatcher from request to forward to target " + target);
	 ...
	 * if not found it tries to load the config from this filter's init parameter
	 * ({@link #loadConfig(FilterConfig)}).
	 * <div>{@inheritDoc}</div>
	 * 
	 ...
	 * @throws ConfigurationException if something failed during
	 *                                initialization/loading of the configuartion
	 */
