 ...
 * </ul>
 * attribute-value: the expected value to match (in order to
 * {@link #evaluate(AuthAttributes)} true
 * 
 * @see Matcher
 * 
 * @author megger
...
public class Requirement extends Matcher implements Rule {
	...
	/**
	 * constructor, initializes this requirement with given attribute name and
	...
	/**
	 * constructor, initializes this requirement with given attribute name, its
	 * expected/required value
	...
	/**
	 * determines whether the given credentials (attributes) meet the requirements
	 * of this instance;
	 * <div>
	 * the behavior depends strongly on the value of {@link #modeMiss}:
	 * <ul>
	 * <li>false (default) - means REQUIRED: evaluate to true if one of the given
	 * attributes matches the requirement,
	 * means (one of its) values must match {@link #value} according to this
	 * instance's {@link Matcher#matchMode}</li>
	 * <li>true - means MISS: expects the attribute(-value) NOT to be there/to be
	 * missed!
	...
	public boolean evaluate(AuthAttributes credentials) {
		...
		for (Object val : attb.getValues()) {
			if (!(val instanceof String))
				// throw new IllegalArgumentException("expecting string attribute, found " +
	 ...
	 * did'nt want to create separate Rule interface for this only class &amp;
	 * method
