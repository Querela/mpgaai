...
/**
 * handles authorization checks for current user to current request (destined
 * location)
...
public class AuthorizationHandler implements Configurable {
	 ...
	 * 
	 * @param authCtx  current authn-context, representing/holding the current user
	 *                 (and session...)
	...
	/**
	 * provides the list of authorization rules which apply (match) for the given
	 * path
	 * 
	 ...
	 * has access (due to his credentials) to the given list of
	...
	private void checkRequirements(AuthenticationContext authCtx, List<Location> locations)
		...
		// ( => no further attributes checks for anonymous fallback user, has no access
		// anyway)
		if (authCtx.isFallback())
			throw new AuthorizationException("requires valid-user for " + locations.toString());
		// evaluate location rules
		// if multiple locations: treat as AND <=> must validate all <=> fail fast (on
		// first)
