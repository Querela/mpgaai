...
/**
 * <p>A login module that loads security information from a SQL database.  Expects
 * to be run by a GenericSecurityRealm (doesn't work on its own).</p>
 * <p>This requires database connectivity information (either 1: a dataSourceName and
 * optional dataSourceApplication or 2: a JDBC driver, URL, username, and password)
 * and 2 SQL queries.</p>
 * <p>The userQuery query should return 2 values, the username and the password in
 ...
 * like: <code>SELECT user, password FROM credentials WHERE username=?</code></p>
 * <p>The groupQuery query should return 2 values, the username and the group name in
 ...
 * <code>SELECT user, role FROM user_roles WHERE username=?</code></p>
 * <p>This login module checks security credentials so the lifecycle methods must return true to indicate success
 * or throw LoginException to indicate failure.</p>
 * 
 * @author megger
 */
