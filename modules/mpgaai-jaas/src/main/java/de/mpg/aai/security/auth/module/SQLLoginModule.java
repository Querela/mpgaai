 ...
 * </p>
 * <p>
 * This requires database connectivity information (either 1: a dataSourceName
 * and optional dataSourceApplication or 2: a JDBC driver, URL, username, and
 * password) and 2 SQL queries.
 * </p>
 * <p>
 ...
 * like: <code>SELECT user, password FROM credentials WHERE username=?</code>
 * </p>
 * <p>
 * The groupQuery query should return 2 values, the username and the group name
 * in that order (but it may return multiple rows, one per group). It should
 * include one PreparedStatement parameter (a ?) which will be filled in with
 * the
 * username. In other words, the query should look like:
 * <code>SELECT user, role FROM user_roles WHERE username=?</code>
 * </p>
 * <p>
 * This login module checks security credentials so the lifecycle methods must
...
public class SQLLoginModule extends AbstractDigestModule implements LoginModule {
	 ...
	 *         or throw a LoginException such as FailedLoginException if
	 ...
	 * 
	 * @param all flag to indicate whether to delete all collected principals, if
	 *            false just groups are cleared
	 */
